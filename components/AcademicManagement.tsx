  ...
  return (
    <div className={`relative flex items-center justify-center ${className}`}>
      <input
        ...
        className={`${isSmall ? 'w-11 h-7 rounded-md text-[10px]' : 'w-14 h-8 rounded-lg text-xs'} px-1 text-center font-black outline-none transition-all border ${isSaved
...
const AcademicManagement: React.FC<AcademicManagementProps> = ({ subjects, setSubjects, onUpdateSyllabus, onUpdateAssessments, students, activeTenant, setCurrentView }) => {
  ...
  const getStudentScore = (subjectId: string, term: string, subType: 'TEST' | 'EXAM' | 'ASSIGNMENT', studentId: string) => {
  ...
  const calculateTermAverage = (subjectId: string, studentId: string, term: string) => {
    const sub = subjects.find(s => s.id === subjectId);
    if (!sub || !sub.assessments) return 0;

    const termAssessments = sub.assessments.filter(a => a.term === term);
    ...
    termAssessments.forEach(asm => {
      const score = asm.scores[studentId] ?? 0;
      const weightedScore = (score / asm.maxMarks) * asm.weightage;
      totalWeightedScore += weightedScore;
      totalWeightage += asm.weightage;
    });

    if (totalWeightage === 0) return 0;
    return Math.round((totalWeightedScore / totalWeightage) * 100);
  };
  ...
  const handleInitializeGrades = (subjectId: string) => {
    ...
    TERMS.forEach(term => {
      baseAssessments.push({
        id: `ASM-${term}-ASSIGN-${Date.now()}`,
        title: `${term} Term Assignment`,
        type: 'ASSIGNMENT',
        term: term,
        subType: 'ASSIGNMENT',
        maxMarks: 100,
                        ...
                        <tr className="bg-slate-50 border-b border-slate-200">
                          {TERMS.map(term => (
                            <React.Fragment key={`${term}-sub`}>
                              <th className="px-3 py-3 text-center text-[9px] font-black text-slate-400 uppercase tracking-tighter border-r border-slate-200">Assign</th>
                        ...
                        {students.filter(s => s.grade === activeSubject?.grade).map(student => {
                          ...
                          return (
                            <tr key={student.id} className="hover:bg-slate-50/80 transition-colors group">
                              ...
                              {TERMS.map(term => {
                                const assignmentData = getStudentScore(selectedGradebookSubjectId, term, 'ASSIGNMENT', student.id);
                                ...
                                return (
                                  <React.Fragment key={`${term}-cells`}>
                                    <td className="px-2 py-4 border-r border-slate-100">
                                      {assignmentData ? (
                                        <GradeInput
                                          initialValue={assignmentData.score ?? ""}
                                          maxMarks={100}
                                          onSave={(v) => handleScoreUpdate(selectedGradebookSubjectId, assignmentData.id, student.id, v)}
                                        />
              ...
              <div className="space-y-4" onDragOver={handleDragOver}>
                {(selectedSubjectForSyllabus.syllabus || []).sort((a, b) => a.order - b.order).map((unit, index) => (
                  <div
                    ...
                    className={`group p-6 rounded-[2rem] border transition-all flex items-start gap-5 relative ${unit.status === 'COMPLETED' ? 'bg-emerald-50/40 border-emerald-100 shadow-sm' :
          ...
          <div className="relative w-full max-w-xl bg-white rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in duration-300 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar space-y-10">
              ...
              <div className="space-y-6">
                {TERMS.map(term => {
                  const assignment = getStudentScore(activeSubject.id, term, 'ASSIGNMENT', selectedStudentForGrades.id);
                      ...
                      <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-3">
                          <div className="flex items-center justify-between px-1">
                            <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Assignment (100)</span>
                          </div>
                          {assignment && (
                            <GradeInput
                              className="w-full"
                              initialValue={assignment.score ?? ""}
                              maxMarks={100}
                              onSave={(v) => handleScoreUpdate(activeSubject.id, assignment.id, selectedStudentForGrades.id, v)}
                            />
                          )}
                        </div>
                        <div className="space-y-3">
