...
const StudentManagement: React.FC<StudentManagementProps> = ({ students, setStudents, activeTenant, admissions }) => {
  ...
  const handleUpdateAdvisor = async (newAdvisor: string) => {
  ...
  const handleUpdateClass = async (grade: string, section: string, stream?: AcademicStream) => {
    if (!selectedStudent) return;
  ...
  const handleAddStudent = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // Sync the real ID if it was generated by DB
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      ...
      {React.cloneElement(children, {
        className: `w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl outline-none text-sm font-medium transition-all focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 ${(children.props as any)?.className || ""}`
      } as any)}
    </div>
  );
}
