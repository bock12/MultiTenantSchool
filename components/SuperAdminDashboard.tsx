...
interface SuperAdminDashboardProps {
  ...
  onProvisionTenant: (tenant: Omit<Tenant, 'id' | 'studentCount'>) => Promise<void>;
}
...
const SuperAdminDashboard: React.FC<SuperAdminDashboardProps> = ({ tenants, onSelectTenant, onProvisionTenant }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await onProvisionTenant(newTenant);
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-10">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-8 py-3 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-slate-800 transition-all"
          >
      ...
      {showProvisionModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" onClick={() => setShowProvisionModal(false)}></div>
          <div className="bg-white rounded-[3rem] w-full max-w-2xl shadow-2xl relative overflow-hidden animate-in fade-in zoom-in duration-300">
            <div className="p-10">
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Provision New Instance</h3>
                  <p className="text-slate-500 text-sm font-medium">Initialize a new school environment in Nexus Cloud</p>
                </div>
                <button onClick={() => setShowProvisionModal(false)} className="p-3 hover:bg-slate-100 rounded-2xl text-slate-400">
                  <X size={24} />
                </button>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-slate-900"
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-slate-900 pr-24"
                      />
                      <span className="absolute right-6 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-xs">.edunexus.io</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Deployment Region</label>
                    <select
                      value={newTenant.region}
                      onChange={e => setNewTenant({ ...newTenant, region: e.target.value })}
                      className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-slate-900 appearance-none"
                    >
                      <option>North District</option>
                      <option>West Metropolitan</option>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-4">
                      <input
                        ...
                        className="w-14 h-14 rounded-xl border-none p-0 cursor-pointer overflow-hidden"
                      />
                      <span className="font-mono text-xs font-bold text-slate-500">{newTenant.primaryColor.toUpperCase()}</span>
                    </div>
                  </div>
                </div>

                <div className="pt-6">
                  <button type="submit" className="w-full py-5 bg-indigo-600 text-white rounded-[2rem] font-black uppercase text-xs tracking-[0.2em] shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all">
                    Initialize Cluster
                  </button>
                </div>
              </form>
